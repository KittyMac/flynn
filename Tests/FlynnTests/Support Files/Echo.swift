...
class Echo: RemoteActor {
    ...
    private func _beTestFailReturn(_ returnCallback: @escaping (String) -> Void) {
...
extension Echo {
    ...
    struct BeTestFailReturnCodableResponse: Codable {
        let response0: String
    }

    @discardableResult
    public func bePrintThreadName(_ sender: Actor,
                                  _ error: @escaping () -> Void,
                                  _ callback: @escaping (Int) -> Void) -> Self {
        unsafeSendToRemote("Echo", "bePrintThreadName", Data(), sender, error) {
            callback(
    ...
    public func bePrint(_ string: String) -> Self {
        ...
        unsafeSendToRemote("Echo", "bePrint", data, nil, nil, nil)
        return self
    }
    @discardableResult
    public func beToLower(_ string: String,
        ...
        // swiftlint:disable:next force_try
        let data = try! JSONEncoder().encode(msg)
        unsafeSendToRemote("Echo", "beToLower", data, sender, error) {
            callback(
                // swiftlint:disable:next force_try
    ...
    public func beTestDelayedReturn(_ string: String,
        ...
        // swiftlint:disable:next force_try
        let data = try! JSONEncoder().encode(msg)
        unsafeSendToRemote("Echo", "beTestDelayedReturn", data, sender, error) {
            // swiftlint:disable:next force_try
    ...
    @discardableResult
    public func beTestFailReturn(_ sender: Actor,
                                 _ error: @escaping () -> Void,
                                 _ callback: @escaping (String) -> Void) -> Self {
        unsafeSendToRemote("Echo", "beTestFailReturn", Data(), sender, error) {
            // swiftlint:disable:next force_try
            let response = (try! JSONDecoder().decode(BeTestFailReturnCodableResponse.self, from: $0))
            callback(
                response.response0
            )
        }
    ...
    public func unsafeRegisterAllBehaviors() {
        ...
        safeRegisterDelayedRemoteBehavior("beTestFailReturn") { [unowned self] (_, callback) in
            self._beTestFailReturn {
                callback(
                    // swiftlint:disable:next force_try
                    try! JSONEncoder().encode(
                        BeTestFailReturnCodableResponse(
                            response0: $0
                        )
                    )
                )
