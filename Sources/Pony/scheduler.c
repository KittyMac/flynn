...
// ---------------------------------------------------------------------------
// Diagnostics: answer "why is this actor not running?" from inside the process,
// so it works on a stripped release build with no debugger attached.
// ---------------------------------------------------------------------------
void pony_actor_debug_dump(void* actor_)
{
    pony_actor_t* actor = (pony_actor_t*)actor_;
    if(actor == NULL) { pony_syslog2("FlynnDbg", "actor is NULL"); return; }

    pony_msg_t* head = atomic_load_explicit(&actor->queue.head, memory_order_acquire);
    int empty_bit = ((uintptr_t)head & 1) ? 1 : 0;

    // Count messages actually linked into the queue, independent of the
    // num_messages counter (which markempty() stores 0 into and which can skew).
    int linked = 0;
    pony_msg_t* n = actor->queue.tail;
    if(n != NULL) n = atomic_load_explicit(&n->next, memory_order_acquire);
    while(n != NULL && linked < 100000) {
        linked++;
        n = atomic_load_explicit(&n->next, memory_order_acquire);
    }

    pony_syslog2("FlynnDbg",
        "actor uid=%d head=%p empty_bit=%d linked_msgs=%d num_messages=%d "
        "suspended=%d parked=%d yield=%d affinity=%d batch=%d",
        actor->uid, (void*)head, empty_bit, linked,
        (int)atomic_load_explicit(&actor->queue.num_messages, memory_order_relaxed),
        (int)atomic_load_explicit(&actor->suspended, memory_order_acquire),
        (int)atomic_load_explicit(&actor->parked, memory_order_acquire),
        (int)atomic_load_explicit(&actor->yield, memory_order_relaxed),
        actor->coreAffinity, actor->batchSize);

    int total = 0, c;
    c = ponyint_mpmcq_contains(&inject, actor);               total += c;
    pony_syslog2("FlynnDbg", "  inject               n=%d contains=%d",
                 (int)ponyint_mpmcq_num_messages(&inject), c);
    c = ponyint_mpmcq_contains(&injectHighEfficiency, actor); total += c;
    pony_syslog2("FlynnDbg", "  injectHighEfficiency n=%d contains=%d",
                 (int)ponyint_mpmcq_num_messages(&injectHighEfficiency), c);
    c = ponyint_mpmcq_contains(&injectHighPerformance, actor);total += c;
    pony_syslog2("FlynnDbg", "  injectHighPerf       n=%d contains=%d",
                 (int)ponyint_mpmcq_num_messages(&injectHighPerformance), c);

    for(uint32_t i = 0; i < scheduler_count; i++) {
        c = ponyint_mpmcq_contains(&scheduler[i].q, actor);   total += c;
        pony_syslog2("FlynnDbg",
            "  sched[%u] aff=%s parked=%d idle=%d terminate=%d q=%d contains=%d",
            i,
            scheduler[i].coreAffinity == kCoreAffinity_OnlyEfficiency ? "E" : "P",
            (int)atomic_load_explicit(&scheduler[i].parked, memory_order_acquire),
            (int)atomic_load_explicit(&scheduler[i].idle, memory_order_relaxed),
            (int)atomic_load_explicit(&scheduler[i].terminate, memory_order_relaxed),
            (int)ponyint_mpmcq_num_messages(&scheduler[i].q), c);
    }

    pony_syslog2("FlynnDbg",
        "  schedulers=%u sleeping=%d running=%d e_cores=%d p_cores=%d => in_queue=%d",
        scheduler_count,
        (int)atomic_load_explicit(&sleeping_count, memory_order_relaxed),
        (int)atomic_load_explicit(&schedulers_running, memory_order_acquire),
        (int)ponyint_e_core_count(), (int)ponyint_p_core_count(), total);

    if(empty_bit == 0 && total == 0) {
        pony_syslog2("FlynnDbg",
            "  VERDICT: ORPHANED -- empty_bit clear so no push will ever schedule "
            "it, but it is in no queue. linked_msgs=%d will never run.", linked);
    } else if(empty_bit == 1 && linked > 0) {
        pony_syslog2("FlynnDbg",
            "  VERDICT: queue marked empty but %d messages are linked -- "
            "markempty/push race.", linked);
    } else if(total > 0) {
        pony_syslog2("FlynnDbg", "  VERDICT: queued and waiting for a scheduler.");
    } else {
        pony_syslog2("FlynnDbg", "  VERDICT: idle, queue genuinely empty.");
    }
}
