...
static void pool_block_push(pool_block_t* block)
{
    ...
    while(true)
    {
        ...
        pool_block_t* check_pos = atomic_load_explicit(&prev->global, memory_order_relaxed);
...
{
    pool_block_t* block = atomic_load_explicit(&pool_block_global.global, memory_order_relaxed);
    ...
    while(true)
    {
        block = atomic_load_explicit(&pool_block_global.global, memory_order_relaxed);
        ...
        pool_block_t* check_block = atomic_load_explicit(&prev->global, memory_order_relaxed);
        ...
        pool_block_t* next = atomic_load_explicit(&block->global, memory_order_relaxed);
...
static void pool_push(pool_local_t* thread, pool_global_t* global)
{
    ...
    assert((p->length > 0) && (p->length <= global->count));
...
static pool_item_t* pool_pull(pool_local_t* thread, pool_global_t* global)
{
    ...
    assert((top->length > 0) && (top->length <= global->count));
...
#define BITS (64 - POOL_MIN_BITS)
    ...
    if(size > POOL_MIN)
        return (size_t)BITS - __builtin_clzl(size) - (!(size & (size - 1)));
